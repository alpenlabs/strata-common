...
/// Builds a commit marker script `OP_RETURN <magic || tail>` for fixtures.
fn build_marker_script(magic: &MagicBytes, tail: &[u8]) -> ScriptBuf {
    let mut payload = Vec::with_capacity(magic.as_bytes().len() + tail.len());
    payload.extend_from_slice(magic.as_bytes());
    payload.extend_from_slice(tail);
    Builder::new()
        .push_opcode(OP_RETURN)
        .push_slice(PushBytesBuf::try_from(payload).expect("marker within push limit"))
        .into_script()
}
...
/// Parses a fixture commit with [`TEST_MAGIC`].
#[cfg(test)]
pub(crate) fn parse_commit(commit: &Transaction) -> ParsedCommit {
