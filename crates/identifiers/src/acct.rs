...
impl AccountSerial {
    ...
    /// Returns the next serial number, incrementing by one.
    ///
    /// # Panics
    ///
    /// If the serial number is already at the maximum value.
    ...
    /// Returns `true` if this serial falls within the system-reserved range.
